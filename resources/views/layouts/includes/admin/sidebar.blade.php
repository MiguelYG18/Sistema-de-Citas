                        ...
                        @isset($link['submenu'])
                            ...
                            <ul id="dropdown-example" class="hidden py-2 space-y-2">
                                @foreach ($link['submenu'] as $item)
                                    <li>
                                        <a href="{{$item['href']}}" class="flex items-center w-full p-2 text-white transition duration-75 rounded-lg pl-11 group hover:bg-sky-400">{{$item['name']}}</a>
                                    </li>
                                @endforeach
                            </ul>
                        @else
                            <a href="{{$link['href']}}" class="flex items-center p-2 text-white rounded-lg hover:bg-sky-400 group {{$link['active'] ? 'bg-sky-400' : ''}}">
                                <span class="w-6 h-6 inline-flex justify-center items-center text-white">
                                    <i class="{{$link['icon']}}"></i>
                                </span>
